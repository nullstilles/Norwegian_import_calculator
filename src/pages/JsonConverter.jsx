        ...
        <div className="max-w-7xl mx-auto px-4 py-8 h-[calc(100vh-8rem)] flex flex-col">
            <div className="flex items-center justify-between mb-6">
                <h1 className="text-2xl font-bold text-white flex items-center gap-2">
                    <FileJson className="text-nordic-accent" /> Developer Converter
                </h1>
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 h-full">
                {/* JSON Input */}
                <div className="flex flex-col h-full bg-nordic-card rounded-xl border border-slate-700/50 overflow-hidden">
                    <div className="bg-slate-800/50 px-4 py-3 border-b border-slate-700/50 flex justify-between items-center">
                        <span className="font-mono text-sm text-nordic-accent font-bold">input.json</span>
                        <div className="flex gap-2">
                            <button onClick={prettify} className="text-xs bg-slate-700 hover:bg-slate-600 text-white px-2 py-1.5 rounded transition">Prettify</button>
                            <button onClick={minify} className="text-xs bg-slate-700 hover:bg-slate-600 text-white px-2 py-1.5 rounded transition">Minify</button>
                            <button onClick={() => setJsonInput('')} className="text-slate-400 hover:text-red-400 transition"><Trash2 size={16} /></button>
                        </div>
                    </div>
                    <textarea
                        className="flex-1 w-full bg-transparent p-4 font-mono text-sm text-slate-300 resize-none outline-none focus:bg-slate-800/30 transition-colors"
                        placeholder='Paste JSON here... e.g. [{"id":1, "name":"Test"}]'
                        value={jsonInput}
                ...
                {/* CSV Output */}
                <div className="flex flex-col h-full bg-nordic-card rounded-xl border border-slate-700/50 overflow-hidden relative group">
                    ...
                    <div className="bg-slate-800/50 px-4 py-3 border-b border-slate-700/50 flex justify-between items-center">
                        <span className="font-mono text-sm text-green-400 font-bold">output.csv</span>
                        <div className="flex gap-2">
                            <button onClick={convertToJson} className="text-xs bg-slate-700 hover:bg-slate-600 text-white px-2 py-1.5 rounded transition">CSV to JSON</button>
                            <button onClick={() => copyToClipboard(csvOutput)} className="text-slate-400 hover:text-green-400 transition" title="Copy to Clipboard"><Copy size={16} /></button>
                        </div>
                    </div>
                    <textarea
                        className="flex-1 w-full bg-transparent p-4 font-mono text-sm text-slate-300 resize-none outline-none focus:bg-slate-800/30 transition-colors"
